...
from employees)
union all
(select count(shipper_ID) as shipper_count
from shippers)
union all
...
from orders)
union all
(select count(order_detail_id) as order_detail_count
from order_details);
...
INSERT INTO Product_Dim (Product_Name, Supplier_ID, Category_ID, Unit, Price)
...
INSERT INTO Order_Detail_Dim (Order_ID, Product_ID, Quantity)
SELECT Order_ID,
Product_ID,
Quantity
From order_details;

Select * from Order_Detail_Dim;

INSERT INTO Customer_Dim (Customer_Name, Contact_Name, Address, City, Postal_Code, Country)
SELECT
    Customer_Name,
    Contact_Name,
    Address,
...
INSERT INTO Employee_Dim (Last_Name, First_Name, Birth_Date, Photo, Notes)
SELECT
    Last_Name,
    First_Name,
    Birth_Date,
...
INSERT INTO Shipper_Dim (Shipper_Name, Phone)
SELECT
    Shipper_Name,
    Phone
FROM Shippers;
...
(
    ...
    FOREIGN KEY (Order_ID) REFERENCES Orders (Order_ID),
	FOREIGN KEY (Order_detail_ID) REFERENCES Order_detail_dim (Order_detail_ID),
    FOREIGN KEY (Product_ID) REFERENCES Product_Dim (Product_ID),
    FOREIGN KEY (Customer_ID) REFERENCES Customer_Dim (Customer_ID),
    FOREIGN KEY (Employee_ID) REFERENCES Employee_Dim (Employee_ID),
    FOREIGN KEY (Shipper_ID) REFERENCES Shipper_Dim (Shipper_ID)
);

select *
from sales_fact;

INSERT INTO Sales_Fact (Order_ID, Order_Detail_ID, Order_Date, Customer_ID, Employee_ID, Shipper_ID, Price, Quantity, Product_ID)
SELECT
    o.Order_ID,
	od.Order_Detail_ID,
    o.Order_Date,
...
(select count(product_id) as prod_dim
from product_dim)
union all
(select count(employee_id) as empl_dim
from employee_dim)
union all
(select count(shipper_id) as shipp_dim
from shipper_dim)
union all
(select count(customer_id) as cust_dim
from customer_dim)
union all
(select count(order_detail_id) as ord_dim
from order_detail_dim)
union all
(select count(fact_id) as sales_fact
from sales_fact);



SELECT c.category_name, cd.Country, ROUND(SUM(s.Quantity * s.Price)) as total_dollar_sales
FROM sales_fact s
JOIN product_dim pd ON pd.product_ID = s.product_id
JOIN customer_dim cd ON cd.customer_id = s.customer_id
JOIN categories c ON pd.category_id = c.category_id
GROUP BY c.category_name, cd.Country
ORDER BY ROUND(SUM(s.Quantity * s.Price)) desc;
